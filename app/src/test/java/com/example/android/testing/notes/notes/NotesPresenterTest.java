...
//    this test validates that the presenter correctly displays a list of
//    notes from the repo and update the view with progress indicator and
//    notes display
    @Test
    public void loadNotesFromRepositoryAndLoadIntoView() {
        // removed call to fail because we are implementing the test
        // fail("Implement in step 6");

        // Given an initialized NotesPresenter with initialized notes
        // When loading of Notes is requested
        // tell the presenter to load notes from the repo, the result is a callback
        mNotesPresenter.loadNotes(true);

        // Callback is captured and invoked with stubbed notes
        // check that the method has been called by the presenter
    ...
    //this test verifies if the Presenter adds a new note, then the results is a request
    // for the view to show the new add-note screen
    @Test
